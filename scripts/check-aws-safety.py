...
PROOF_SIDECARS = (
    ROOT / "scripts/submission-proof.py",
    ROOT / "scripts/build-submission-proof.py",
    ROOT / "scripts/judge-proof.py",
)
...
def main() -> int:
    ...
    if any(path.exists() for path in PROOF_SIDECARS):
        fail("submission proof bundles must be generated by aws_nitro_runner, not sidecar scripts")
    ...
    require(
        runner,
        "write_submission_proof_bundle(self.config.artifact_dir)",
        "successful smoke must write a self-contained submission proof bundle after post-audit",
    )
    require(runner, "def run_proof(", "main AWS runner must expose a read-only proof regeneration command")
    require(runner, "SUBMISSION_PROOF_BUNDLE", "AWS runner must name the submission proof bundle artifact")
    require(runner, "SUBMISSION_PROOF_BUNDLE_SHA256", "AWS runner must write a bundle checksum")
    require(runner, "SUBMISSION_EVIDENCE_FILES", "AWS runner must keep a first-class required evidence file list")
    for proof_artifact in (
        ...
        require(runner, proof_artifact, f"submission proof bundle must include {proof_artifact}")
    require(runner, "MANIFEST.sha256", "submission proof bundle must include a manifest of bundled evidence hashes")
    require(runner, "tarfile.open", "submission proof must bundle evidence files, not only point at loose paths")
    require(runner, "_assert_no_secret_markers", "submission proof must reject credential-like evidence")
